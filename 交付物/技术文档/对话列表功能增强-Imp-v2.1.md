...
| Step | 位置 | 操作 | 说明 |
|------|------|------|------|
| 1 | `server/src/db/schema.ts` | 修改 | conversations 表加 pinned 列 |
| 2 | `server/src/routes/conversations.ts` | 修改 | GET / 排序逻辑加 pinned DESC |
| 3 | `agenthub-mvp/src/types.ts` | 修改 | Conversation 加 pinned 字段 |
| 4 | `agenthub-mvp/src/api/client.ts` | 修改 | 新增 updateConversation / deleteConversation |
| 5 | `agenthub-mvp/src/store/appStore.ts` | 修改 | 3个新action + 排序逻辑 |
| 6 | `agenthub-mvp/src/components/ConversationList.tsx` | 修改 | 置顶分区 + 三点菜单入口 |
| 7 | `agenthub-mvp/src/components/ConversationMenu.tsx` | **新增** | 下拉菜单组件 |
